...
def test_lente_integration_profile_and_discovery_are_controlled() -> None:
    client = TestClient(app)

    profile_response = client.get("/api/v1/integrations/apps/lente")
    discovery_response = client.get("/api/v1/integrations/apps/lente/discovery")
    profile = profile_response.json()
    discovery = discovery_response.json()

    assert profile_response.status_code == 200
    assert profile["app_id"] == "lente"
    assert profile["integration_status"] == "prepared_for_discovery"
    assert profile["external_connection_enabled"] is False
    assert profile["contract_id"] == "lente.discovery.v1"

    assert discovery_response.status_code == 200
    assert discovery["app_id"] == "lente"
    assert discovery["contract_id"] == "lente.discovery.v1"
    assert discovery["evidence_source"] in {
        "runtime_repository",
        "versioned_local_discovery_snapshot",
    }
    assert discovery["evidence_files_found"]
    assert discovery["external_connection_enabled"] is False
    assert "No Lente runtime connection is enabled from ecosystem-foundation." in discovery["blockers"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "web_factory",
            "web_factory.discovery.v1",
            "No Web Factory runtime connection is enabled from ecosystem-foundation.",
        ),
        (
            "marketing",
            "marketing.discovery.v1",
            "No Marketing runtime connection is enabled from ecosystem-foundation.",
        ),
        (
            "marca_personal",
            "marca_personal.discovery.v1",
            "No Marca Personal runtime connection is enabled from ecosystem-foundation.",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "forja",
            "forja.discovery.v1",
            "No FORJA runtime connection is enabled from ecosystem-foundation.",
        ),
        (
            "cerebro",
            "cerebro.discovery.v1",
            "No CEREBRO runtime connection is enabled from ecosystem-foundation.",
        ),
    ],
)
def test_block_4_integration_profiles_and_discovery_are_controlled(
    app_id: str,
    contract_id: str,
    blocker: str,
) -> None:
    ...
    assert discovery["health_status"] in {
        "local_evidence_found",
        "local_evidence_snapshot_found",
    }
    assert discovery["missing_evidence_files"] == []
    assert discovery["evidence_files_found"] == discovery["evidence_files_expected"]
    assert discovery["external_connection_enabled"] is False
