...
def test_pluma_contract_is_seeded_without_external_connection() -> None:
    response = client.get("/api/v1/contracts", params={"app_id": "pluma"})
    contracts = response.json()

    assert response.status_code == 200
    assert any(item["id"] == "pluma.discovery.v1" for item in contracts)
    pluma_contract = next(
        item for item in contracts if item["id"] == "pluma.discovery.v1"
    )
    assert pluma_contract["status"] == "prepared_for_discovery"
...
def test_block_2_contracts_are_seeded_without_external_connection(
    app_id: str,
    contract_id: str,
) -> None:
...
def test_block_3_contracts_are_seeded_without_external_connection(
    app_id: str,
    contract_id: str,
) -> None:
    response = client.get("/api/v1/contracts", params={"app_id": app_id})
