...
def assert_block4_discovery(base_url: str, token: str) -> None:
    ...
    for app_id, contract_id in BLOCK_4_APPS.items():
        ...
        assert_field(registry, "status", "planned", app_id)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate authenticated FORJA/CEREBRO production state.")
    ...
    assert_block4_discovery(base_url, token)
    print(":: FORJA discovery profile PASS")
    print(":: CEREBRO discovery profile PASS")
